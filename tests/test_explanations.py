from shapley_values.explanations import (
    ...
    split_suggestion_explanation,
)
import numpy as np


def test_largest_conflict():
    data = np.array([[0, 2, 4], [-2, 0, 5], [1, 4, 0]], dtype=float)
    ...
    data_all_pos = np.array([[0, 2, 4], [2, 0, 5], [1, 4, 0]], dtype=float)

    _, p1_pos, p2_pos = largest_conflict(data_all_pos)

    assert (p1_pos, p2_pos) == (-1, -1)

    data_all_neg = np.array([[0, -2, -4], [-2, 0, -5], [-1, -4, 0]], dtype=float)
...
def test_why_worst():
    ...
    shap_values = np.array([[-1, 2, 3], [-2, -1, 2], [-2, -2, -1]], dtype=float)
    ...
    shap_values_mixed = np.array([[-1, 10, 3], [-2, -1, 2], [-2, -2, -1]], dtype=float)
...
def test_why_best():
    ...
    actual_mixed = np.array([-3, 1, -3], dtype=float)
    shap_values_mixed = np.array([[-1, 10, 3], [-2, -1, 2], [-2, -5, -1]], dtype=float)
...
def test_why_objective_i():
    shap_values = np.array([[1, 2, 3], [-2, -1, 2], [-2, -3, -1]], dtype=float)
...
def test_how_to_improve_objective_i():
    ...
    shap_values = np.array(
        [[-1, 2, -3, 1], [-2, -3, -1, 2], [2, 1, 5, 3], [0, 0, 0, -2]], dtype=float
    )
    ...
    objective_i = 0
    msg, improve, impair, case = how_to_improve_objective_i(
        shap_values, objective_i, target, actual_worse
    )
    ...
    objective_i = 0
    msg, improve, impair, case = how_to_improve_objective_i(
        shap_values, objective_i, target, actual_better
    )
    ...
    shap_values_missing = np.array(
        [[-1, -2, -3, -4], [-2, -1, -1, 2], [2, 1, 5, 3], [1, 5, 2, 1]], dtype=float
    )
    ...
    msg, improve, impair, case = how_to_improve_objective_i(
        shap_values_missing, objective_i, target, actual_neutral
    )
    ...
    shap_values_itself = np.array(
        [[-10, -2, 3, -4], [-2, -5, -1, 2], [2, 1, 5, 3], [-1, 5, -2, 8]], dtype=float
    )
    ...
    msg, improve, impair, case = how_to_improve_objective_i(
        shap_values_itself, objective_i, target, actual_neutral
    )
    ...
    shap_values_no_best = np.array(
        [[10, 2, 13, 4], [-2, -5, -1, 2], [2, 1, 5, 3], [-1, 5, -2, 8]], dtype=float
    )

    objective_i = 0

    msg, improve, impair, case = how_to_improve_objective_i(
        shap_values_no_best, objective_i, target, actual_neutral
    )
    ...
    msg, improve, impair, case = how_to_improve_objective_i(
        shap_values_no_worst, objective_i, target, actual_neutral
    )
    ...
    shap_values_no_best_self = np.array(
        [[19, 2, 13, 4], [-2, -1, -3, -4], [2, 1, 5, 3], [-1, 5, -2, 8]], dtype=float
    )

    objective_i = 0

    msg, improve, impair, case = how_to_improve_objective_i(
        shap_values_no_best_self, objective_i, target, actual_neutral
    )
    ...
    shap_values_no_worst_self = np.array(
        [[19, 2, 13, 4], [-2, -1, -3, -4], [2, 1, 5, 3], [-4, -5, -2, -1]], dtype=float
    )

    objective_i = 3

    msg, improve, impair, case = how_to_improve_objective_i(
        shap_values_no_worst_self, objective_i, target, actual_neutral
    )
...
def test_split():
    explanation_part = "This is the explanation part."
    suggestion_part = "This is the suggestion part."
    input_s = explanation_part + "\n" + suggestion_part

    output_t = split_suggestion_explanation(input_s)

    assert output_t[0] == suggestion_part
    assert output_t[1] == explanation_part

    incorrect_input = "No new line is bad!"

    output_t2 = split_suggestion_explanation(incorrect_input)

    assert output_t2[0] == output_t2[1]
